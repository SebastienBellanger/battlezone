...
(function() {
  var AxisModel, BackgroundHorizonModel, BackgroundMountainsModel, BattleZone, BulletModel, BulletNode, Camera, Matrix3, Matrix4, Model, Node, OrthoProjection, Pipeline, Projection, Quaternion, Renderer, TankBodyModel, TankNode, TankRadarModel, Tester, Transform, Vector3, Vector4,
  ...
  Renderer = (function() {
    ...
    function Renderer() {
      ...
      projection = new Projection(this.width, this.height, 1, 1000, 80.0);
  ...
  BattleZone = (function() {
    function BattleZone() {
      ...
      this.rotation = 0;
      this.rootNode.addChild(new Node(new BackgroundHorizonModel));
      this.rootNode.addChild(new Node(new BackgroundMountainsModel));
      for (i = l = 0; l < 10; i = ++l) {
        tank = new TankNode();
        ...
        this.rootNode.addChild(tank);
      }
      this.rootNode.addChild(new TankNode);
      this.rootNode.addChild(new Node(new AxisModel));
      this.position = new Vector3(0, 5, 30);
    ...
    BattleZone.prototype.update = function() {
      ...
      this.last = now;
      if (this.moveLeft) {
        this.rotation += -step * 0.002;
      }
      if (this.moveRight) {
        this.rotation += step * 0.002;
      }
      if (this.moveForward) {
        this.position.addThis(Vector3.UNITZ.mul(-step * 0.02));
      }
      if (this.moveBackward) {
        this.position.addThis(Vector3.UNITZ.mul(step * 0.02));
      }
      if (this.moveUp) {
        this.position.addThis(Vector3.UNITY.mul(step * 0.02));
      }
      if (this.moveDown) {
        this.position.addThis(Vector3.UNITY.mul(-step * 0.02));
      }
      if (this.shoot) {
        ...
        this.rootNode.addChild(bullet);
        this.shoot = false;
      }
      this.renderer.clear();
      this.renderer.drawText(10, 20, this.position.negate());
      this.renderer.pipeline.viewMatrix.setTranslationV(this.position.negate());
      this.renderer.pipeline.recalculateTransform();
      this.renderer.pipeline.addTransform(Quaternion.fromAxisAngle(Vector3.UNITY, this.rotation).toMatrix());
  ...
  BulletNode = (function(superClass) {
    ...
    BulletNode.prototype.onUpdate = function(step) {
      return this.transform.translateZ(-step * 0.05);
    };
  ...
  TankNode = (function(superClass) {
    ...
    TankNode.prototype.onUpdate = function(step) {
      return this.radarNode.transform.rotateY(step * 0.005);
    };
